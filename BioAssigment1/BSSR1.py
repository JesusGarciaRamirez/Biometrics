...
def read_BSSR1_scores_from_file(files_type,enrollees_id_filepath, users_id_filepath, path):
...
# replace the following two lines of code with the previous line if the tqdm package is not installed
    ...
    # #Saving to disk data
    # df.to_pickle("{}_df.pkl".format(files_type))

    with open("{}_data_arr.pkl".format(files_type), 'wb') as f:
        pickle.dump(dataframe, f)
    with open("{}_enrollees.pkl".format(files_type), 'wb') as f:
        pickle.dump(enrollees, f)
    with open("{}_users.pkl".format(files_type), 'wb') as f:
        pickle.dump(users, f)
    with open("{}_column_names.pkl".format(files_type), 'wb') as f:
...
if __name__=="__main__":
    ...
    read_BSSR1_scores_from_file("ri",ri_enrollees_id_filepath, ri_users_id_filepath, ri_scores_list_str)